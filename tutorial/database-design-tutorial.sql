...
CREATE TABLE customer(
    customer_id serial NOT NULL,  -- serial = auto numbered
...
CREATE TABLE service(
    ...
    name varchar(100) NOT NULL,
    cost numeric(7,2) NOT NULL, -- numeric allows 5 digits to left of decimal and 2 to right
...
**************************************************************************/
ALTER TABLE pet -- only written here to practice alter statements
...
CREATE TABLE pet_service(
    ...
    CONSTRAINT FK_pet_service_pet FOREIGN KEY(pet_id) REFERENCES pet (pet_id), -- name is key type and both tables together
...
SELECT p.name AS "Pet Name", p.type AS "Pet Type", c.name AS "Owner", ps.service_date AS "Date", s.name AS "Service"
    ...
    JOIN pet_service ps ON p.pet_id = ps.pet_id
